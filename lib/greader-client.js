...
class GreaderClient {
  ...
  quickAdd(url, T) { return this.post('/reader/api/0/subscription/quickadd', { quickadd: url, T }, { parseJson: true }); }
  subscriptionExport() { return this.getText('/reader/api/0/subscription/export'); }
  async subscriptionImport(opml, T) {
    // Some implementations (Miniflux) require the anti-CSRF token and accept
    // OPML as a form field; FreshRSS expects the OPML as the raw XML body.
    // Try the token/form shape first when available, then fall back to raw XML.
    if (T) {
      const form = await this.post('/reader/api/0/subscription/import', { opml, T });
      if (form.status >= 200 && form.status < 300) return form;
    }
  ...
  streamItemIds(streamId, query) {
    return this.getJson('/reader/api/0/stream/items/ids', { output: 'json', s: streamId, ...query });
  }

  streamItemsContents(ids, order, T) {
    return this.post('/reader/api/0/stream/items/contents', { output: 'json', i: ids, r: order, T }, { parseJson: true });
  }
